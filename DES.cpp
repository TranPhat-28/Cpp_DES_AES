...
string ModeList[] = {"ECB", "CBC", "OFB", "CFB", "CTR"};
...
int main()
{
	...
	{
		wcin >> input_mode;
	} while (input_mode < 1 || input_mode > 5);
	system("cls");
	...
	else if (input2 == 2)
	{
		...
		wstring skey1;
		wcout << "Please input key (8 bytes): " << endl;
		...
		ss1.Pump(8);  // Pump first 16 bytes

		// Input IV from terminal
		wstring siv1;
		wcout << "Please input IV (8 bytes): " << endl;
		...
		ss2.Pump(8);  // Pump first 16 bytes
...
void printMenu()
{
    wcout << "ENCRYPT and DECRYPT using DES: " << endl;
